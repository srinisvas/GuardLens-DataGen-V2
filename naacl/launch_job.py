...
def main():
    ...
    try:
        ...
        command=[sys.executable,str(HERE/'run_stage.py'),stage,'--input',str(input_path),
            ...
            '--record-workers',str(positive_env('RECORD_WORKERS',max(8,nt*4,nj*4))),
            '--intervention-workers',str(positive_env('INTERVENTION_WORKERS',max(4,nt*4))),
            '--target-inflight',str(positive_env('TARGET_INFLIGHT',2)),
            '--judge-inflight',str(positive_env('JUDGE_INFLIGHT',4))]
        for role,endpoints in urls.items():
